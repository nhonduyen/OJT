...
namespace OJT
{
    public class COURSE
    {
        ...
        public virtual int Insert(string FROM_MONTH,string TO_MONTH)
        {
            var sql = "INSERT INTO COURSE(FROM_MONTH,TO_MONTH) VALUES(@FROM_MONTH,@TO_MONTH)";
            return DBManager<COURSE>.Execute(sql, new { FROM_MONTH = FROM_MONTH,TO_MONTH = TO_MONTH});
        }

        public virtual int Update(int ID, string FROM_MONTH, string TO_MONTH)
        {
            var sql = "UPDATE COURSE SET FROM_MONTH=@FROM_MONTH,TO_MONTH=@TO_MONTH WHERE ID=@ID";

            return DBManager<COURSE>.Execute(sql,  new { ID = ID,FROM_MONTH = FROM_MONTH,TO_MONTH = TO_MONTH});
        }
