...
namespace OJT
{
    public class HISTORY
    {
        ...
        public List<dynamic> GetHistory(string mentor = "", string mentee = "", int course_id = 0)
        {
           
            var sql = string.Format(@"
SELECT
(SELECT COUNT(1) FROM HIS_DETAIL AS D1 WHERE D1.EMP_ID=H.EMP_ID AND (@MENTOR='' OR MENTOR=@MENTOR) AND (@EMP_ID='' OR EMP_ID=@EMP_ID) AND (@COURSE_ID=0 OR COURSE_ID=@COURSE_ID) GROUP BY D1.EMP_ID) AS CNT_EMP,
(SELECT COUNT(1) FROM HIS_DETAIL AS D1 WHERE D1.EMP_ID=H.EMP_ID AND D1.COURSE_ID=H.COURSE_ID AND (@MENTOR='' OR H.MENTOR=@MENTOR) AND (@EMP_ID='' OR H.EMP_ID=@EMP_ID) AND (@COURSE_ID=0 OR H.COURSE_ID=@COURSE_ID) GROUP BY D1.COURSE_ID) AS CNT_COURSE,
E.NAME AS EMP_NAME, PICTURE,DEPARTMENT,H.MENTOR AS MENTOR1,
...
");
        ...
        public int CountHistory(string mentor = "", string mentee = "", int course_id = 0)
        {
            var sql = string.Format(@"
SELECT COUNT(1)
FROM COURSE AS C
